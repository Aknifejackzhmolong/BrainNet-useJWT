...
public interface UserRepository extends JpaRepository<User,Integer> {
    ...
    @Query(value = "SELECT a FROM User a WHERE  a.email = ?1 AND a.valid = 1")
    User findUserByEmail(String email);

    @Query(value = "SELECT a FROM User a WHERE  a.name = ?1 AND a.valid = 1")
    User findUserByName(String name);

    @Query(value = "SELECT a FROM User a WHERE a.valid = 0")
    Page<User> findUsers(Pageable pageable);

    @Query(value = "SELECT a FROM User a WHERE a.valid = 0")
    Page<User> findFreezeUsers(Pageable pageable);

    @Query(value = "SELECT a FROM User a WHERE a.id = ?1 AND a.valid = 1")
    User findUserById(Integer id);

    @Query(value = "SELECT a FROM User a WHERE a.id = ?1 AND a.valid = 0")
